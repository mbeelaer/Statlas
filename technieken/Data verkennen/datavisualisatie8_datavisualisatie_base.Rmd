...
Op deze pagina focussen we op de praktische kant: hoe kan je data visualiseren met R? Er zijn al heel wat mogelijkheden met de basisversie van R ("base R"), maar er bestaat ook een package `ggplot2` dat speciaal ontworpen is met het oog op datavisualisatie. Dat biedt nog meer mogelijkheden, maar is ook wat moeilijker om aan te leren. 

We voorzien hieronder een hele korte introductie tot de algemene principes van `ggplot2` om vervolgens te demonstreren hoe je verschillende types grafieken kan ontwerpen (histogram, boxplot,...). Er is geen ellenlange uitleg voorzien bij elk type. Soms vind je in het lichtblauwe blok wat commentaar bij de code zelf (rechts van de `#`). 
